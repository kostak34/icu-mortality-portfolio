...
- Upload a single patient .txt
- OR enter observations manually at 12h / 6h / Now

Important:
- Demo / education only. Not for clinical use.
- Strict mode (default): refuses to predict if insufficient data.
- Estimate mode: missing model inputs are filled using typical training values (via imputer),
  and this is clearly disclosed to the user.
"""
...
STRICT_MIN_PRESENT_FEATURES = 20  # tune if needed
STRICT_MIN_MEASUREMENTS_ENTERED = 4  # tune if needed
...
def engineer_from_manual(
    ...
    return feats, measurements_entered, missing_by_time
...
# IMPORTANT FIX:
# Do NOT set session_state for the same key you're using in the widget with a default.
method = st.radio(
    label="",
    options=["Upload patient file", "Manual entry"],
    index=1,  # keep Manual entry default as before (change to 0 if you want upload default)
    horizontal=True,
)
...
with st.expander("What this tool does", expanded=False):
    st.write(
        "This demo estimates mortality risk from ICU-style observations. "
        "It’s designed to show how a model can turn common measurements into a risk estimate."
    )
    st.write(
        "It is **not** validated for clinical use, and should not be used to make real decisions."
    )

with st.expander("What data can I upload?", expanded=False):
    st.write(
        "Upload a single patient `.txt` file in the same public dataset format used to train this demo. "
...
if method == "Upload patient file":
    ...
    try:
        ...
        if strict_mode and present_count < STRICT_MIN_PRESENT_FEATURES:
            st.error(
        ...
        if not missing_pretty:
            ...
            st.write(
                "Missing inputs were filled using typical training values (Estimate mode)."
                if not strict_mode
                else "Some model inputs were missing. Strict mode only proceeds if enough data is present overall."
            )
...
if method == "Manual entry":
    ...
    def _row_numeric(base: str, key_prefix: str):
        ...
        v12 = c1.text_input("", key=f"{key_prefix}_{base}_12h", label_visibility="collapsed")
        v6 = c2.text_input("", key=f"{key_prefix}_{base}_6h", label_visibility="collapsed")
        vnow = c3.text_input("", key=f"{key_prefix}_{base}_now", label_visibility="collapsed")
    ...
    if st.button("Calculate risk", type="primary"):
        try:
            feats, measurements_entered, missing_by_time = engineer_from_manual(manual_inputs)
            X_imp, present_count, missing_features, X_aligned = align_and_impute(feats, feature_columns, imputer)

            st.subheader("Data check")
            st.write(f"Measurements entered: **{measurements_entered}**")
            st.write(f"Model features available (derived): **{present_count}/{len(feature_columns)}**")

            if strict_mode and (present_count < STRICT_MIN_PRESENT_FEATURES or measurements_entered < STRICT_MIN_MEASUREMENTS_ENTERED):
                st.error(
